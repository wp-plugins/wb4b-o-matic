...
/*
 * Plugin Name: WB4B-o-Matic
 * Description: Modifications of WP-o-Matic, a plugin that enables administrators to create posts automatically from RSS/Atom feeds.
 * Author: Daniel Oliveira after Guillermo Rauch
 * Plugin URI: http://wb4b.com.br/
 * Version: 1.0RC4-6
 * =======================================================================

 * This mod of WP-o-Matic allows the user to add the author and the url of the author of the post that was imported
 * using the starred item on google reader. Translated to Brazilian Portuguese (PT-Br). No other modification was made.
...
class WPOMatic {               
  ...
  {              
    ...
    # Table names init
    $this->db = array(
      'campaign'            => $wpdb->prefix . 'wb4bo_campaign',
      'campaign_category'   => $wpdb->prefix . 'wb4bo_campaign_category',
      'campaign_feed'       => $wpdb->prefix . 'wb4bo_campaign_feed',     
      'campaign_word'       => $wpdb->prefix . 'wb4bo_campaign_word',   
      'campaign_post'       => $wpdb->prefix . 'wb4bo_campaign_post',
      'log'                 => $wpdb->prefix . 'wb4bo_log'
    );                                    
    ...
    # Is installed ?
    $this->installed = get_option('wb4bo_version');
    $this->setup = get_option('wb4bo_setup');
    
    # Actions
    add_action('activate_wb4b-o-matic/wb4bomatic.php', array(&$this, 'activate'));                # Plugin activated
    add_action('deactivate_wb4b-o-matic/wb4bomatic.php', array(&$this, 'deactivate'));            # Plugin deactivated
    ...
    $this->adminurl = $this->optionsurl . '?page=wb4bomatic.php';
    $this->pluginpath = get_option('siteurl') . '/wp-content/plugins/wb4b-o-matic';           
    $this->helpurl = $this->pluginpath . '/help.php?item=';
    $this->tplpath = $this->pluginpath . '/inc/admin';
    $this->cachepath = WPODIR . get_option('wb4bo_cachepath');
    
    # Cron command / url
    $this->cron_url = $this->pluginpath . '/cron.php?code=' . get_option('wb4bo_croncode');
  ...
  function activate($force_install = false)
  {
    ...
    WPOTools::addMissingOptions(array(
     'wb4bo_log'          => array(1, 'Gravar Log das ações do WB4B-o-Matic'),
     'wb4bo_log_stdout'   => array(0, 'Mostar o log enquanto a campanha é processada'),
     'wb4bo_unixcron'     => array(WPOTools::isUnix(), 'Usar unix-style cron'),
     'wb4bo_croncode'     => array(substr(md5(time()), 0, 8), 'Cron job password.'),
     'wb4bo_cacheimages'  => array(0, 'Gravar em cache todas as imagens. Sobrescreve as opções da campanha'),
     'wb4bo_cachepath'    => array('cache', 'Armazena em cache o caminho relativo do plugin')
    ));
    ...
    dbDelta(  "CREATE TABLE {$this->db['campaign_post']} (
     ...
     dbDelta(  "CREATE TABLE {$this->db['log']} (
      ...
      add_option('wb4bo_version', $this->version, 'Log da versão instalada');
  ...
  {   
    ...
    // Delete options
    WPOTools::deleteOptions(array('wb4bo_log', 'wb4bo_log_stdout', 'wb4bo_unixcron', 'wb4bo_croncode', 'wb4bo_cacheimages', 'wb4bo_cachepath'));
  }                                
  ...
  function log($message)
  {
    ...
    if(get_option('wb4bo_log_stdout'))
      echo $message;    
    
    if(get_option('wb4bo_log'))
    {
  ...
  function getCommand()
  {
    ...
    return WPOTools::pick($commands[0], $commands[1], $commands[2], $commands[3], '<em>{wget ou comando similar aqui}</em>');
  }
  ...
  function filterPermalink($url)
  {
    ...
    if(get_the_ID())
    {
    	$campaignid = (int) get_post_meta(get_the_ID(), 'wb4bo_campaignid', true);

    	if($campaignid)
    	{
    	  $campaign = $this->getCampaignById($campaignid);
    	  if($campaign->linktosource)
    	    return get_post_meta(get_the_ID(), 'wb4bo_sourcepermalink', true);
    	}  	  
  ...
  function processCampaign(&$campaign)
  {
    ...
    // Log 
    $this->log('Processando campanha ' . $campaign->title . ' (ID: ' . $campaign->id . ')');
  ...
  function processFeed(&$campaign, &$feed)
  {
    ...
    // Log
    $this->log('Processando feed ' . $feed->title . ' (ID: ' . $feed->id . ')');
    ...
    foreach($simplepie->get_items() as $item)
    {
      ...
      {
        if($count == 0) $this->log('Nenhum post novo');
        break;
      }        
      
      if($this->isDuplicate($campaign, $feed, $item))
      {
        $this->log('Filtrando posts duplicados');
        break;
      }
      ...
      {
        $this->log('Limite de importação da campanha atingido em ' . $campaign->max);
        break;
      }
    }
  ...
  function processItem(&$campaign, &$feed, &$item)
  {
   ...
   if ($author = $item->get_author())
	...
	{
		$blog =  $source->get_title();
	}

			
    // Meta
    $meta = array(
      'wb4bo_campaignid' => $campaign->id,
      'wb4bo_feedid' => $feed->id,
      'wb4bo_sourcepermalink' => $item->get_permalink(),
      'wb4bo_sourceauthor' => $autor,
	  'wb4bo_sourceblog' => $blog
    );  
  ...
  {  
    ...
    // Caching
    if(get_option('wb4bo_cacheimages') || $campaign->cacheimages)
    {
  ...
  function cacheRemoteImage($url)
  {
    ...
    { 
      ...
      return $this->pluginpath . '/' . get_option('wb4bo_cachepath') . '/' . $filename;
    }
  ...
  function adminWarning()
  {
    ...
    {
      echo "<div id='wpo-warning' class='updated fade-ff0000'><p>" . sprintf(__('Por favor <a href="%s">clique aqui</a> iniciar e configurar o WB4B-o-Matic.', 'wpomatic'), $this->adminurl . '&amp;s=setup') . "</p></div>
  ...
  function adminMenu()
  {
    add_submenu_page('options-general.php', 'WB4B-o-Matic', 'WB4B-o-Matic', 10, basename(__FILE__), array(&$this, 'admin'));
  }                    
  ...
  function adminSetup()
  {
    ...
    {
      update_option('wb4bo_unixcron', isset($_REQUEST['option_unixcron']));
      update_option('wb4bo_setup', 1);
  ...
  {  
    ...
    {              
      update_option('wb4bo_unixcron',     isset($_REQUEST['option_unixcron']));
      update_option('wb4bo_log',          isset($_REQUEST['option_logging']));
      update_option('wb4bo_log_stdout',   isset($_REQUEST['option_logging_stdout']));      
      update_option('wb4bo_cacheimages',  isset($_REQUEST['option_caching']));
      update_option('wb4bo_cachepath',    rtrim($_REQUEST['option_cachepath'], '/'));
      ...
      else 
      {
        switch($_REQUEST['import_mode'])
        {
          ...
          case '1':
            ...
            $this->add_success = __('Campanhas adicionadas com sucesso. Sinta-se livre para editá-las', 'wpomatic');
          ...
          case '2':
            ...
            $this->add_success = sprintf(__('Feeds adicionados com sucesso. <a href="%s">Editar campanha/a>', 'wpomatic'), $this->adminurl . '&s=edit&id=' . $campaignid);
  ...
  function adminExportProcess()
  {
    if($_POST)
    {
      ...
      {
        $this->export_error = __('Por favor, selecione pelo menos uma campanha', 'wpomatic');
      } else 
      {
        $campaigns = array();
  ...
  {  
    ...
    {
      $errors['basic'][] = __('Você precisa adicionar um título para a campanha', 'wpomatic');
      $this->errno++;
    }
    ...
    if(!$feedscount)
    {
      $errors['feeds'][] = __('Você precisa cadastrar pelo menos um feed', 'wpomatic');
      $this->errno++;
    } else {  
      if(isset($this->campaign_data['feeds']['new']))    
      {
        foreach($this->campaign_data['feeds']['new'] as $feed)
        {
          ...
          if($simplepie->error())
          {
            $errors['feeds'][] = sprintf(__('Feed <strong>%s</strong> não pode ser importado (SimplePie error: %s)', 'wpomatic'), $feed, $simplepie->error());
            $this->errno++;
          }          
        }  
      }
    ...
    {
      $errors['categories'][] = __('Selecione pelo menos uma categoria', 'wpomatic');
      $this->errno++;
    }
    ...
    if(sizeof($this->campaign_data['rewrites']))
    {
      foreach($this->campaign_data['rewrites'] as $rewrite)
      {
        if($rewrite['origin']['regex'])
        {
          ...
          {
            $errors['rewrites'][] = __('Ocorreu um erro com a RegEx fornecida', 'wpomatic');         
            $this->errno++;
          }
        }
      }
    ...
    {
      $errors['options'][] = __('Username do autor não encontrado', 'wpomatic');
      $this->errno++;
    }
    
    if(! $this->campaign_data['main']['frequency'])
    {
      $errors['options'][] = __('Frequência selecionada não é válida', 'wpomatic');
      $this->errno++;
    }
    
    if(! ($this->campaign_data['main']['max'] === 0 || $this->campaign_data['main']['max'] > 0))
    {
      $errors['options'][] = __('Máximo de itens deve ser um número válido (maior que zero)', 'wpomatic');
      $this->errno++;
    }
    
    if($this->campaign_data['main']['cacheimages'] && !is_writable($this->cachepath))
    {
      $errors['options'][] = sprintf(__('Caminho do cache (em <a href="%s">Options</a>) deve estar com permissão de escrita antes de ativar o cache das imagens.', 'wpomatic'), $this->adminurl . '&s=options' );
      $this->errno++;
    }
  ...
  function adminProcessTools()
  {
    ...
    {      
      ...
      $this->tool_success = __('Todos posts removidos', 'wpomatic');
      return true;
    }
    ...
    if(isset($_REQUEST['tool_changetype']))
    {
      ...
      $this->tool_success = __('Status do post atualizado', 'wpomatic');
      return true;
    }
    ...
    if(isset($_REQUEST['tool_changeauthor']))
    {
      ...
      if($author)
      {
        ...
        $this->errno = 1;
        $this->errors = array('tools' => array(sprintf(__('Autor %s não encontrado', 'wpomatic'), attribute_escape($_REQUEST['campaign_tool_changeauthor']))));
      }
